...
void populate(std::shared_ptr<BaseInstance> instance, QMenu* menu)
{
    ...
    QObject::connect(normalLaunchOffline, &QAction::triggered, [instance] { APPLICATION->launch(instance, false, false); });
    QObject::connect(normalLaunchDemo, &QAction::triggered, [instance] { APPLICATION->launch(instance, false, true); });
    ...
    QObject::connect(profilers, &QActionGroup::triggered, [mcInstance](QAction* action) {
        mcInstance->settings()->set("Profiler", action->data());
        // Since we are external, we can't emit profilerChanged() directly if it's protected. (It is a signal).
        // However, we changed the setting, and MinecraftInstance usually reacts to setting changes or the UI does.
        // The original code emitted it explicitly: emit profilerChanged();
        // PackProfile/InstanceWindow listens to this?
        // Let's assume setting change is enough or we might need to invoke it via metaobject if needed,
        // but cleaner is if InstanceWindow listens to "Profiler" setting change.
        // For now, to match exact behavior without friending, we rely on settings update.
    });
