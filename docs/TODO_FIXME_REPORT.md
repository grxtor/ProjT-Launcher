...
- **FIXME** in `launcher/meta/JsonFormat.cpp`:39 — // FIXME: remove this from here... somehow
```
...
- **FIXME** in `launcher/meta/JsonFormat.h`:73 — // FIXME: this has a different shape than the others...FIX IT!?
```
...
- **TODO** in `launcher/minecraft/OneSixVersionFormat.cpp`:151 — // added for legacy Minecraft window embedding, TODO: remove
```
...
- **FIXME** in `launcher/minecraft/Rule.h`:80 — // FIXME: unsupported
```
    struct OS {
        QString name;
        // FIXME: unsupported
...
- **FIXME** in `launcher/minecraft/launch/PrintInstanceInfo.h`:43 — // FIXME: temporary wrapper for existing task.
```
...
- **FIXME** in `launcher/minecraft/launch/ExtractNatives.h`:41 — // FIXME: temporary wrapper for existing task.
```
...
- **TODO** in `launcher/ui/widgets/InfoFrame.cpp`:190 — // TODO: Wrap links inside <a> tags
```
